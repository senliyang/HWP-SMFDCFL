...
A = pd.read_csv('interaction.txt', header=None, index_col=None,sep='\t').values

feature_MFm, feature_MFd = get_low_feature(D, 0.01, pow(10, -4), A.T)

feature_MFm = pd.DataFrame(feature_MFm)
feature_MFm.to_csv('NMF_32_microbe_feature.csv',header=False,index=False)

feature_MF = pd.DataFrame(feature_MFd)
feature_MF.to_csv('NMF_32_disease_feature.csv',header=False,index=False)